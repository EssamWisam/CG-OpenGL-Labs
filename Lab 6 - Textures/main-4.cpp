...
int main(int, char**) {
    ...
    //* 1 - Set the magnification filter texture parameter to nearest
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);  
    // The smiley face used to be blurry because its an 8x8 image and we're using much more pixels
    // to view it. By default a magnifying filter with option linear is used which interpolates (weight avg.) the
    // color of the pixel based on the corresponding large texture pixels around it.
    // Meanwhile the "nearest" approach simple check at which large texture pixel does the pixel
    // to be colored fall and gives it the same color.
    // i.e. "take value of the texture element that is nearest to the center of the pixel being textured."


    
    //!Meanwhile suppose the pixel being textured maps to an area greater than one texture element.
    // then we can set what to do using the GL_TEXTURE_MIN_FILTER parameter
    // It's this time nearest by default (compare coordinate of real pixel with the coordinates of texture pixels)
    // and takes nearest. Many other options as well: https://www.khronos.org/registry/OpenGL-Refpages/es2.0/xhtml/glTexParameter.xml
