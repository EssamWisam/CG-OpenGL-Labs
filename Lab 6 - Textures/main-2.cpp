...
struct Vertex {
    float x, y, z;
    uint8_t r, g, b, a;
    float s, t;                     // part of "introduce texel coordinates"                      
...
int main(int, char**) {
    ...
    //* 1 - Introduce the texture coordinates (last two columns). Check the vertex struct above.
    Vertex vertices[] = {
    ...
    // Texture coordinates range from (0,0) bottom left of texture to (1,1) top right of texture.
    // so with this setup, each of the four vertices of the cube is matched against 
    // the corresponding vertex in the smily face.
    ...
    //* 2 - Send the texture coordinates (just like how we did it with position and color)
    //{_______________________________________
    GLint texCoordLoc = 2; 
    glEnableVertexAttribArray(texCoordLoc);
    glVertexAttribPointer(texCoordLoc, 2, GL_FLOAT, false, sizeof(Vertex), (void*)offsetof(Vertex, s));
    // offset is s so we start reading after passing by float x, y, z; uint8_t r, g, b, a;
    // and we jump by the size of vertex to get the next texture coordinate

    //* 3 - Receive it in vertex shader and propagate it to fragment shader.

    // check texture.vert and corrected-texture.frag
