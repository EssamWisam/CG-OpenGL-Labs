...
int main(int, char**) {
    ...
    GLuint fs = loadShader("assets/shaders/simple4.frag", GL_FRAGMENT_SHADER);
...
void drawMonkey(int W, int H, GLuint program, Mesh mesh){
        ...
        //* 6 - Send the needed light uniforms from the main. (Camera Rotation was turned of as well.)
        glUniform1i(glGetUniformLocation(program, "light.type"), 2);              // Spotlight.
        glUniform3f(glGetUniformLocation(program, "light.position"), 2, 0, 0);    // Coming from the right.     
        glUniform3f(glGetUniformLocation(program, "light.direction"), -1, 0, 0);   // pointing to the left.
        glUniform3f(glGetUniformLocation(program, "light.attenuation"), 1, 0, 0);   // Realistic Attenuation
        glUniform2f(glGetUniformLocation(program, "light.cone_angles"), glm::radians(10.0f), glm::radians(11.0f));   

        glUniform3f(glGetUniformLocation(program, "light.diffuse"), 1, 0.9, 0.7);
        glUniform3f(glGetUniformLocation(program, "light.specular"), 1, 0.9, 0.7);      
        glUniform3f(glGetUniformLocation(program, "light.ambient"), 0.2, 0.18, 0.14);   
