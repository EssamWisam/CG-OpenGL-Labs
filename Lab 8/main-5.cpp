...
int main(int, char**) {
    ...
    GLuint fs = loadShader("assets/shaders/simple3.frag", GL_FRAGMENT_SHADER);
...
void drawMonkey(int W, int H, GLuint program, Mesh mesh){
       ...
       //* 8 - Send the uniform for eye position to vertex shader
        ...
        // The third argument (true) will take care of the needed transpose.



       //* 9 - Send to the fragment shader the light and material structs.

        glUniform3f(glGetUniformLocation(program, "light.direction"), 0, -1, 0);        // Going vertically down.
        glUniform3f(glGetUniformLocation(program, "light.diffuse"), 1, 0.9, 0.7);
        glUniform3f(glGetUniformLocation(program, "light.specular"), 1, 0.9, 0.7);      // Same as diffuse (yellowish light)
        glUniform3f(glGetUniformLocation(program, "light.ambient"), 0.2, 0.18, 0.14);   // Weaker version of them.

        glUniform3f(glGetUniformLocation(program, "material.diffuse"), 0.2, 0.3, 0.4);  // Make it low and make specular high (shiney monkey)
        glUniform3f(glGetUniformLocation(program, "material.specular"), 0.8, 0.9, 1.0); // High specular
        glUniform3f(glGetUniformLocation(program, "material.ambient"), 0.2, 0.3, 0.4);  // Just like diffuse
